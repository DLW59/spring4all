...
import com.dlw.study.service.ProdPointMessageService;
...
@Service
@Slf4j
public class ProdPointMessageServiceImpl implements ProdPointMessageService {

    /**
     * 可售
     */
    @Value("${setpoint.service.ssp.point.prod.new}")
    private String prodNewPointUrl;

    /**
     * 不可售
     */
    @Value("${setpoint.service.ssp.point.prod.disable}")
    private String prodDisableUrl;

    /**
     * 禁忌
     */
    @Value("${setpoint.service.ssp.point.prod.taboo}")
    private String prodTabooUrl;
    ...
    private void taboo(String messageText) {
        ...
        try {
            ...
            restTemplate.postForObject(prodTabooUrl,taboo,Response.class);
        }catch (Exception e){
            return;
        }
    ...
    private void disable(String messageText) {
        ...
        try {
            ...
            restTemplate.delete(prodDisableUrl,disable);
        }catch (Exception e){
            return;
        }
        log.info("消费消息：{}",messageText);
    ...
    private void newPoint(String messageText) {
        ...
        try {
            ...
            restTemplate.put(prodNewPointUrl,add,Response.class);
        }catch (Exception e){
            return;
        }
        log.info("消费消息：{}",messageText);
