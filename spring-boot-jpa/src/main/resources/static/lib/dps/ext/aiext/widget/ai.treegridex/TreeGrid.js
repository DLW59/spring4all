 ...
 * @abstract
 */
Ext.ux.maximgb.treegrid.AbstractTreeStore = Ext.extend(Asiainfo.data.AsiaInfoJsonStore,
{
	/**
	...
	{       
		...
		this.addListener('add',function(Store,records,index){ 
		     { alert('add:'+records.length);
			var r=records[0];
			this.cache.save.push(r.data);
		        Store.curRecord = r;
		        Store.itemindex = Store.indexOfId(r.id);
	...
	{      
	 ...
	 var i, len, temp,
         ...
         for(var i=0,cnt= this.getCount();i<cnt;i+=1){
  	       var r=this.getAt(i);
  	       if(r.get(this.parent_id_field_name))continue;
...
Ext.ux.maximgb.treegrid.GridPanel = Ext.extend(Ext.grid.EditorGridPanel, 
{
	...
	{      
		...
		this.colModel= new Ext.grid.ColumnModel({columns:this.columns});
		this.colModel.rows=this.rows; 
	//	this.colModel.rows=this.rows;
