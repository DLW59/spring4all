...
Toolbar.prototype.init = function()
{
	...
	/*this.addItems(['save','saveAs', 'export', 'editFile', 'import', '-'],['glyphicon glyphicon-floppy-disk','glyphicon glyphicon-floppy-save','glyphicon glyphicon-cloud-download','glyphicon glyphicon-edit','glyphicon glyphicon-cloud-upload']);
...
/**
 * Adds a div to the separator.
 */
Toolbar.prototype.addHtml = function(html,tooltip)
{  
	var elt = document.createElement('div');
	$(elt).addClass("geLabel");
	//elt.html=html;
	//mxUtils.write(elt, html);
	$(html).appendTo(elt);
	this.initElement(elt, tooltip); 
	this.container.appendChild(elt);
 ...
 */
Toolbar.prototype.addItems = function(keys,classArray)
{   
	for (var i = 0; i < keys.length; i++)
	{
		...
		{
			if(!classArray) this.addItem('geSprite-' + key.toLowerCase(), key)
			else if(classArray.length>i)this.addItem(classArray[i], key)
			else this.addItem('geSprite-' + key.toLowerCase(), key)
			//this.addItem('glyphicon glyphicon-eye-open',key)
		}
 ...
 * Adds given action item
 */
Toolbar.prototype.addItem = function(sprite, key,caption)
{
	...
	if (action != null)
	{
		elt = this.addButton(sprite, action.label, action.funct,caption);
 ...
 */
Toolbar.prototype.addButton = function(classname, tooltip, funct,caption)
{
	var elt = this.createButton(classname,caption);
 ...
 */
Toolbar.prototype.createButton = function(classname,caption)
{
	...
	if(classname && classname.indexOf('geSprite')>=0)
	  inner.className = 'geSprite ' + classname
	else  inner.className =  classname;
	 
	elt.appendChild(inner);
//	if(caption) elt.appendChild(caption)
	if(caption) mxUtils.write(elt, caption);
	return elt;
};
