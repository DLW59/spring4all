...
{
	 if(this.editorUi.editor.loadwidgetSql){
	 var _store=ai.getJsonStore(this.editorUi.editor.loadwidgetSql);
	 var groups=[];
	 for(var i=0;i<_store.getCount();i++){
	 	 var r=_store.getAt(i);
	 	 var applytype=r.get('APPLY');
	 	 if(groups.indexOf(applytype)<0) groups.push( applytype);
	 };
	
   for(var i=0;i<groups.length;i++){
   	  var groupItem =groups[i];
   	  var expandFlag;
   	  if(groupItem=="1.常用") expandFlag=true;
   	  else expandFlag=false;
   	  this.addPalette(i, groupItem, expandFlag, mxUtils.bind(this, function(content){
   	  	// console.log(content.id);
   	  	 var groupItem=groups[content.id];
   	  	 console.log(this.id+","+this.title);
   	  	 for(var j=0;j<_store.getCount();j++){
   	  	     var r=_store.getAt(j);
   	  	     if(r.get('APPLY')!=groupItem) continue;
   	  	     if(this.editorUi.editor.editortype=='appbuild'){
   	  	       content.appendChild(this.createVertexTemplate('html=1;image;image='+IMAGE_RESOURCES_PATH+'dacp-lib/ext/aiext/flowchar/images/etlwidget/'+r.get('FUNC_ICON'), 165,150,r.get('WIDGETNAME'),r.get('FUNC_CODE')))
   	  	     }
   	  	     else{
   	  	     	content.appendChild(this.createVertexTemplate('image;image='+IMAGE_RESOURCES_PATH+'dacp-lib/ext/aiext/flowchar/images/etlwidget/'+r.get('FUNC_ICON'), 52,40,r.get('WIDGETNAME'),r.get('FUNC_CODE')))
   	  	    };
   	  	 }
	...
	{
		//覆写创建节点函数，调整拖拽图标大小，初始化值
		//新增  state：新建或选择 procType：程序类型
		Sidebar.prototype.createVertexTemplate = function(style, width, height, value, objType, state)
		{
			var cells = [new mxCell((value != null) ? value : '', new mxGeometry(0, 0, width, height), style)];
			
			////对cell的值进行初始化
			cells[0].vertex = true;
			if(objType) cells[0].objType = objType;			
			cells[0].state= state;
			cells[0].cnname='';
			cells[0].xmlid = '';	
			...
			{
				cells[0].agentCode = '';		
				cells[0].mutiRunFlag = '0';	
			}
			 
			return this.createVertexTemplateFromCells(cells, 35, 35);
		};

	 //处理流程设计所定义的对象
	 if(this.editorUi.editor.flowSiderBar)
	 {
	 	var groupItems = this.editorUi.editor.flowSiderBar.groupItems;
	 	var iconPath = this.editorUi.editor.flowSiderBar.iconPath;

	 	var i = 0;
	 	for(key in groupItems) //遍历组
	 	{
	 		this.addPalette(i, key, false, mxUtils.bind(this, function(content){
				var groupItem=groupItems[content.title];
				...
				for(var j=0; j<groupItem.length; j++){
					var subItem = groupItem[j];
					content.appendChild(this.createVertexTemplate(
						'image;image=' + iconPath + '/' + subItem.icon,
						52, 40, subItem.name, subItem.id, state)); //调整为40X40sidebar中文字显示不完整，暂时52X40
				}
	  ...
	  var dir = STENCIL_PATH;
	this.addStencilPalette('basic',"基本形状", dir + '/basic.xml',
		';fillColor=#ffffff;strokeColor=#000000;strokeWidth=2');
	this.addStencilPalette('arrows',"箭头", dir + '/arrows.xml',
		';fillColor=#ffffff;strokeColor=#000000;strokeWidth=2');
	...
	this.addStencilPalette('uielement', '界面原型', dir + '/mockup/all.xml',';fillColor=#ffffff;strokeColor=#000000;strokeWidth=2');
	//this.addStencilPalette('aws/dbse', '数据库', dir + '/pid/valves.xml',';fillColor=#ffffff;strokeColor=#000000;strokeWidth=2');
	 
	this.addImagePalette('clipart', "图形", dir + '/clipart/', '_128x128.png',
...
Sidebar.prototype.addGeneralPalette = function(expand)
{
	...
	this.addPalette('general', "基本形状", expand || true, mxUtils.bind(this, function(content)
	{
		content.appendChild(this.createVertexTemplate('htmltext;html=1', 200, 200, 'htmltext',"tip"));
		content.appendChild(this.createVertexTemplate('swimlane', 200, 200, '循环','for'));
		content.appendChild(this.createVertexTemplate('swimlane', 200, 200, '数据交换','dataExchange'));
		content.appendChild(this.createVertexTemplate('swimlane;horizontal=0', 200, 200, 'Pool','arrayloop'));
		...
		if (cells.length > 0)
		{
			...
			{  
				for(var i=0;i<cells.length;i++){
					...
					if(cells[i].style && cells[i].style.indexOf('html=1')>=0) cells[i].style="html=1";
				};
...
Sidebar.prototype.addGeneralPalette1 = function(expand)
{
	...
	{
		  content.appendChild(this.createVertexTemplate('htmltext', 200, 200, 'Htmltext'));
 ...
 * Creates a drop handler for inserting the given cells.
 */
Sidebar.prototype.createVertexTemplate = function(style, width, height, value,templateId)
{
	var cells = [new mxCell((value != null) ? value : '', new mxGeometry(0, 0, width, height), style)];
	
	////对cell的值进行初始化
	cells[0].vertex = true;
	if(templateId) cells[0].templateId=templateId;
...
Sidebar.prototype.addPalette = function(id, title, expanded, onInit)
{
	...
	div.className = 'geSidebar bs-glyphicons-list';
	div.id=id;
	div.title=title;
	if (expanded)
	{
...
Sidebar.prototype.addImagePalette = function(id, title, prefix, postfix, items)
{
	this.addPalette(id, title, false, mxUtils.bind(this, function(content)
    {
    	for (var i = 0; i < items.length; i++)
		{
			...
			content.appendChild(
			this.createVertexTemplate('image;image=' + icon, 80, 80,title));
		}
    }));
};
