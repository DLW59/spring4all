 ...
 */
Editor = function(graphContainer,graph)
{
	mxEventSource.call(this);
	this.init();
	this.initStencilRegistry();
	this.graph =   new Graph(graphContainer);
 ...
 * load from database.
 */
 Editor.prototype.loadfromDatabase = function(FLOWCODE,METAPRJ){
 	 if(!FLOWCODE) FLOWCODE = paramMap.FLOWCODE;
 	 
 	 if(!METAPRJ) METAPRJ="";
 	 var FLOWNAME = paramMap.FLOWNAME;
 	 if(!METAPRJ) METAPRJ = paramMap.METAPRJ||"";
 	 if(METAPRJ) METAPRJ="_"+METAPRJ;
   if(!FLOWCODE) return ;
   console.log("select FLOWCODE, FLOWNAME,XML,EFF_DATE,CREATER,STATE, STATE_DATE,CURDUTYER, VERSEQ  from TRANSFLOW"+METAPRJ+" where flowcode='"+FLOWCODE+"'");
 ...
 */
Editor.prototype.initStencilRegistry = function(stencilFileName)
{
	// Loads default stencils
 
	if(!stencilFileName) stencilFileName="general.xml";
