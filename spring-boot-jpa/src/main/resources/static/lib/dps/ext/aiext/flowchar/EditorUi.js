...
EditorUi = function(editor, container)
{
	...
	if (this.container == document.body)///wangqs
	{
 ...
 * load from database.
 */
 EditorUi.prototype.loadfromDatabase = function(){
   ...
   this.graphStore = new AI.JsonStore({
			root:'root',
			sql:"select FLOWCODE, FLOWNAME,XML,EFF_DATE,CREATER,STATE, STATE_DATE,CURDUTYER, VERSEQ  from TRANSFLOW"+METAPRJ+" where flowcode='"+FLOWCODE+"'",
			loadDataWhenInit:true,
			table:"TRANSFLOW"+METAPRJ,
			key:"FLOWCODE"
		});
	 }
	 else if(Asiainfo != "undefined"){
	 	 this.graphStore = new Asiainfo.data.AsiaInfoJsonStore({
			root:'root',
			sql:"select FLOWCODE, FLOWNAME,XML,EFF_DATE,CREATER,STATE, STATE_DATE,CURDUTYER, VERSEQ  from TRANSFLOW"+METAPRJ+" where flowcode='"+FLOWCODE+"'",
    ...
    var flowrec;	
    if(this.graphStore.getCount()==0){
     flowrec = this.graphStore.getNewRecord();
     ...
     flowrec.dirty=true;
     this.graphStore.add(flowrec);
     //ds_flow.commit();
    }
	 if (this.graphStore.getCount()!=0)
	 {
	 	var record = this.graphStore.getAt(0);
 ...
 * load from database.
 */
 EditorUi.prototype.savetoDatabase11 = function(xmltext){
	 ...
	 if(this.graphStore.getCount()==0) return;
	 var r=this.graphStore.getAt(0);
	 r.set('STATE_DATE',new Date());
   r.set('CURDUTYER',_UserInfo.usercnname);
	 r.set("XML",xmltext);
	 
	 if(this.graphStore.commit(false)){
	 	alert('保存成功');
	};
};
...
EditorUi.prototype.online = function(){
	if(this.setProcsOnline){
		this.setProcsOnline();
	}
};

EditorUi.prototype.offline = function(){
	if(this.setProcsOffline){
		this.setProcsOffline();
	}
};
...
EditorUi.prototype.addSelectionListener = function()
{
	var selectionListener = mxUtils.bind(this, function()
    {
		...
		var actions = ['cut', 'copy', 'delete', 'duplicate', 'bold', 'italic', 'style', 'fillColor',
		               ...
		               'autosize','online','offline'];
    	
    	for (var i = 0; i < actions.length; i++)
    	{
    		this.actions.get(actions[i]).setEnabled(selected);
    	}
    	this.actions.get('online').setEnabled(true);
    	this.actions.get('offline').setEnabled(true);
