...
var artDialog = function (config, ok, cancel) {
	...
	//解决弹出框样式问题
	if(['Alert','Confirm','Tips','Notice'].indexOf(config.id)!=-1) {
		defaults.padding = "20px 25px"; 
		//解决谷歌浏览器下弹出框出现滚动条的问题
		config.overflow = "hidden";
	}else{
		defaults.padding = "0px 0px"; 
...
artDialog.fn = artDialog.prototype = {
	...
	_init: function (config) {
		...
		DOM.content.css('overflow', config.overflow);
...
artDialog.defaults = {								
	...
	ico:'../public/images/logo.png',//默认标题小图标
